...
{
  var dest_sheet = SpreadsheetApp.openById(dest_ID).getSheetByName("Down Time Tracker Data (AS1)");
...
function SummarizeData(shift_time)
{
  ...
  for(index = 3; index < pivot_table.getLastRow(); index++)
  {
    ...
    switch (reason) 
    {
      case "Late From Start":
        dest_range = dest_sheet_summary.getRange(18,2);
        dest_range.setValue(pivot_table.getRange(index,3).getValue());
        break;
      case "2P/4P Changeover":
        dest_range = dest_sheet_summary.getRange(20,2);
        dest_range.setValue(pivot_table.getRange(index,3).getValue());
        break;
      case "Missing Assembly Ingredient":
        dest_range = dest_sheet_summary.getRange(21,2);
        dest_range.setValue(pivot_table.getRange(index,3).getValue());
        break;
      case "Missing Meal-kits":
        dest_range = dest_sheet_summary.getRange(22,2);
        dest_range.setValue(pivot_table.getRange(index,3).getValue());
        break;
      case "Tape Machine Down":
        dest_range = dest_sheet_summary.getRange(23,2);
        dest_range.setValue(pivot_table.getRange(index,3).getValue());
        break;         
      case "Change Tape":
        dest_range = dest_sheet_summary.getRange(24,2);
        dest_range.setValue(pivot_table.getRange(index,3).getValue());
        break;
      case "P2L System Issues":
        dest_range = dest_sheet_summary.getRange(25,2);
        dest_range.setValue(pivot_table.getRange(index,3).getValue());
        break;
      case "Rework":
        dest_range = dest_sheet_summary.getRange(26,2);
        dest_range.setValue(pivot_table.getRange(index,3).getValue());
        break;
      case "Other":
        dest_range = dest_sheet_summary.getRange(27,2);
        dest_range.setValue(pivot_table.getRange(index,3).getValue());
        break; 
      case "10 mins Break":
  ...
  dest_sheet_summary.getRange(19,2).setValue(total_lateness);
}